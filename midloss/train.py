...
def main(
    ...
    #这里他自己写一个compose是为了把两个transform一起做？
    train_transforms = torchvision.transforms.Compose(train_img_transforms + tensor_transforms)
    train_dataset = obj_factory(train_dataset, transform=train_transforms)
    if val_dataset is not None:
        val_img_transforms = obj_factory(val_img_transforms) if val_img_transforms is not None else []
        val_transforms = torchvision.transforms.Compose(val_img_transforms + tensor_transforms)
    ...
    for epoch in range(start_epoch, epochs):
        # Training step
        epoch_loss, epoch_acc = proces_epoch(train_loader, train=True)

        # Validation step
        if val_loader is not None:
            with torch.no_grad():
                running_metrics.reset()
                epoch_loss, epoch_acc = proces_epoch(val_loader, train=False)
        running_metrics.reset()
